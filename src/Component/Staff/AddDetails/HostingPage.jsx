...
const HostingPage = ({ hostings, setHostings }) => {
  const handleChange = (index, field, value) => {
    const updatedHostings = [...hostings];
    updatedHostings[index][field] = value;
    setHostings(updatedHostings);
  };

  const handleAddHosting = () => {
    setHostings([...hostings, { provider: '', expiryDate: '', plan: '', loginUrl: '' }]);
  };

  const handleRemoveHosting = (index) => {
    setHostings(hostings.filter((_, i) => i !== index));
  };

  return (
    <div className="space-y-8 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h2
        className="text-4xl font-extrabold text-gray-900 dark:text-gray-100 mb-6 border-b border-gray-300 dark:border-gray-700 pb-3 select-none"
      >
        Hostings
      </h2>
      ...
      {hostings.map((hosting, index) => (
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[
              { label: 'Provider', field: 'provider', placeholder: 'Enter provider' },
              { label: 'Expiry Date', field: 'expiryDate', type: 'date' },
              { label: 'Plan', field: 'plan', placeholder: 'Enter plan' },
              { label: 'Login URL', field: 'loginUrl', placeholder: 'Enter login URL' }
